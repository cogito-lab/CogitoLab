...
const types = { '.html': 'text/html; charset=utf-8', '.css': 'text/css', '.js': 'text/javascript', '.svg': 'image/svg+xml', '.xml': 'application/xml', '.txt': 'text/plain', '.png': 'image/png', '.jpg': 'image/jpeg' };

createServer(async (req, res) => {
  try {
    let pathname = decodeURIComponent(new URL(req.url, 'http://localhost').pathname);
    if (pathname === '/' && base) { res.writeHead(302, { Location: `${base}/` }); res.end(); return; }
    if (base && pathname !== base && !pathname.startsWith(`${base}/`)) throw new Error('Outside base');
